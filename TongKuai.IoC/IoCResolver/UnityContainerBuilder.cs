...
namespace TongKuai.IoC.IoCResolver
{
    internal sealed class UnityContainerBuilder
    {
        ...
        public IUnityContainer BuilderUnityContainer()
        {
            ...
            paths.ToList().ForEach(path => this.Configure(container, path));
        ...
        private IEnumerable<string> GetConfigFilePaths()
        {
            ...
            string path = ConfigurationManager.AppSettings["UnityConfigPath"];
            return Directory.GetFiles(HttpContext.Current.Server.MapPath(path))
